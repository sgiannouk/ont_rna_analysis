...
# outdir <- "/Users/stavris/Desktop/Projects/silvia_ont_umc/talon_analysis"
# matrix <- "/Users/stavris/Desktop/Projects/silvia_ont_umc/talon_analysis/raw_talon_abundance.tsv"
# input_groups <- "/Users/stavris/Desktop/Projects/silvia_ont_umc/talon_analysis/talon_input.csv"
# filt_data <- "/Users/stavris/Desktop/Projects/silvia_ont_umc/talon_analysis/filtered_isoforms.csv"
...
# Obtain the transcripts that will remain
iso_to_remain <- data.frame(transcript_ID=read.csv(filt_data, header=F)[,2])
...
print(paste("Applying basic filtering step. Novel transcripts must have at least 8 counts in all samples to be maintained.", sep=" " ))
...
write.csv(filtered_matrix, file=paste(dirname(outdir),"filt_talon_abundance.tsv",sep="/"), quote=F, row.names=F)
...
gene_novelty <- data.frame(dplyr::count(filt_table_overview[!duplicated(filt_table_overview$annot_gene_id), ], gene_novelty))
if (!("Intergenic" %in% gene_novelty$gene_novelty)) { gene_novelty[nrow(gene_novelty) + 1, ] = list("Intergenic", 0) }
transcript_novelty <- data.frame(dplyr::count(filt_table_overview, transcript_novelty))
...
transcript_novelty <- data.frame(Id = 1:length(transcript_novelty$transcript_novelty), Transcript_novelty = transcript_novelty$transcript_novelty, Counts = transcript_novelty$n)
...
for(i in 1:length(samplegroup)) { 
      ...
      gene_novelty <- dplyr::count(selected_filtered_table[!duplicated(selected_filtered_table$annot_gene_id), ], gene_novelty)
      if (!("Intergenic" %in% gene_novelty$gene_novelty)) { gene_novelty[nrow(gene_novelty) + 1, ] = list("Intergenic", 0) }
      transcript_novelty <- dplyr::count(selected_filtered_table, transcript_novelty)
