...
names(results_selected)[1] <- "gene_id"

# Manupulation of expr_file in order to 
# merge add the gene names in the final table
expr_file <- expr_file[ ,c(1,3)]
expr_file <- expr_file[row.names(unique(expr_file[ ,c(1,2)])), ]
row.names(expr_file) <- expr_file$annot_gene_id; expr_file$annot_gene_id <- NULL

# Adding gene_names in the final table
results_selected <- data.frame(merge(expr_file, results_selected, by.x=0, by.y="gene_id"))
# Renaming the first two columns
colnames(results_selected)[1] <- "gene_id"; colnames(results_selected)[2] <- "gene_name"
...
names(total_results)[1] <- "gene_id"
# Adding gene_names in the final table
total_results <- data.frame(merge(expr_file, total_results, by.x=0, by.y="gene_id"))
...
row.names(edger_res_overall) <- edger_res_overall$Row.names; edger_res_overall$Row.names <- NULL; colnames(edger_res_overall)[1] <- "gene_name"
...
colnames(top30_sigNorm)[1] <- "gene_name"
