...
def quality_control(i, seq_summary_file, sample_id, raw_data_dir):
	...
	if i == 1: print("\t{0} QUALITY CONTROL OF THE INPUT SAMPLES".format(datetime.now().strftime("%d.%m.%Y %H:%M")))
...
def mapping_qc():
	...
	print("{0}  RSeQC, Picard, AlignQC & WUB - Generating post-alignment stats: in progress ..".format(datetime.now().strftime("%d.%m.%Y %H:%M")))
	for i, file in enumerate(aligned_data, 1):
		...
		ref_annotation = [refAnnot_ensembl, refAnnot][file.endswith(".genome.bam")]
		...
		"--gtf", "{0}.gz".format(ref_annotation),  # Input reference annotation in gzipped form
