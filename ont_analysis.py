...
refTranscGRCh38 = "/home/stavros/references/reference_transcriptome/GRCh38_GencodeV31/GRCh38_gencode.v31.transcripts_trna.fa"
...
parser.add_argument('-t', '--threads', dest='threads', default=str(5), metavar='', 
...
analysis_dir = os.path.join(current_dir, "TEST")
...
def quality_control(i, seq_summary_file, sample_id, raw_data_dir):
	...
	"--html_outfile", os.path.join(initial_qc_reports, "{0}_pycoQC-report.html".format(sample_id)),  # Create the report in the reports directory
	"--report_title", "\"{0} quality control report\"".format(sample_id),  # A title to be used in the html report
	"2>>", os.path.join(pipeline_reports, "pycoQC-prelim_report.txt")])
...
def preprocessing_raw_data(raw_data_dir):
	...
	"2>>", os.path.join(pipeline_reports, "porechop-report.txt")])
...
def alignment_against_ref(i, sample_id, raw_data_dir):
	...
	subprocess.run('samtools index -@ {0} {1}'.format(args.threads, os.path.join(alignments_dir, "{0}.genome.bam".format(sample_id))), shell=True)
	...
	"2>>", os.path.join(pipeline_reports, "minimap2_transcriptome-report.txt")])  # Directory where all reports reside
...
def mapping_qc():
	...
	for i, file in enumerate(aligned_data, 1):
		...
		"--html_outfile", os.path.join(postAlignment_reports, "{0}.{1}.pycoQC-report.html".format(file_name, aligned_to)),  # Create the report in the reports directory
		"--report_title", "\"Post-alignment quality control report\"",  # A title to be used in the html report
		"2>>", os.path.join(pipeline_reports, "pycoQC-report.txt")])
		subprocess.run(pycoQC, shell=True)
		...
		"> {0}/{1}.{2}.bamstat.txt".format(postAlignment_reports, file_name, aligned_to),  # Output file
		"2>>", os.path.join(pipeline_reports, "{0}_{1}_bamstats-report.txt".format(file_name, aligned_to))])
		...
		"OUTPUT= {0}/{1}.{2}.alignment_metrics.txt".format(postAlignment_reports, file_name, aligned_to),  # Output
		"REFERENCE_SEQUENCE= {0}".format(ref_file),  # Reference sequence file
		"2>>", os.path.join(pipeline_reports, "{0}_{1}_CollectAlignmentSummaryMetrics-report.txt".format(file_name, aligned_to))])
		...
		"--output", "{0}/{1}.{2}.alignqc.xhtml".format(postAlignment_reports, file_name, aligned_to),  # Output pdf file
		file,
		"2>>", os.path.join(pipeline_reports, "{0}_{1}_alignqc-report.txt".format(file_name, aligned_to))])
		...
		"-r", "{0}/{1}.{2}.bam_alignment_qc.pdf".format(postAlignment_reports, file_name, aligned_to),  # Output pdf file
		"-p", "{0}/{1}.{2}.bam_alignment_qc.pk".format(postAlignment_reports, file_name, aligned_to),  # Output pk file
		file,
		"2>>", os.path.join(pipeline_reports, "{0}_{1}_alignment_qc-report.txt".format(file_name, aligned_to))])
		...
		if file.endswith(".genome.bam"):
			...
			"> {0}/{1}.{2}.fragSize".format(postAlignment_reports, file_name, aligned_to),  # Output file
			"2>>", os.path.join(pipeline_reports, "{0}_{1}_read_distribution-report.txt".format(file_name, aligned_to))])
			...
			"-i", file,  # Input BAM file
			"-r", reference_annotation_bed,
			"> {0}/{1}.{2}.strandness.txt".format(postAlignment_reports, file_name, aligned_to),  # Output file
			"2>>", os.path.join(pipeline_reports, "{0}_{1}_strandness-report.txt".format(file_name, aligned_to))])
			...
			"-r", reference_annotation_bed,
			"-i", file,  # Input BAM file
			"-o", "{0}/{1}.{2}.gene_coverage".format(postAlignment_reports, file_name, aligned_to),  # Output file
			"2>>", os.path.join(pipeline_reports, "{0}_{1}_mapping_position-report.txt".format(file_name, aligned_to))])
			...
			"METRICS_FILE= {0}/{1}.{2}.mark_duplicates.txt".format(postAlignment_reports, file_name, aligned_to),  # Output file
			"2>>", os.path.join(pipeline_reports, "{0}_{1}_duplicate_reads-report.txt".format(file_name, aligned_to))])
			...
			file,  # Input BAM file
			"> {0}/{1}.{2}.samtools_idxstats.txt".format(postAlignment_reports, file_name, aligned_to),  # Output file
			"2>>", os.path.join(pipeline_reports, "{0}_{1}_mapping_position-report.txt".format(file_name, aligned_to))])
...
class expression_matrix:
	...
	def novel_transcripts_detection_talon(self, threads):
		...
		if not os.path.exists(csv_file):
			...
			with open(csv_file, "w") as talon_out:
				for path, subdir, folder in os.walk(alignments_dir):
					for file in folder:
						if file.endswith('.genome.bam'):
							talon_out.write('{0},{1},ONT,{2}\n'.format(file.split(".")[0], file.split(".")[0].split("_")[0],os.path.join(path, file).replace(".bam", ".sam")))
		...
		### Initial step: nternal priming check
		print("{0}  1/6 TALON - Run talon_label_reads on each file to compute how likely each read is to be an internal priming product: in progress ..".format(datetime.now().strftime("%d.%m.%Y %H:%M")))
		...
		for file in genome_alignments:
			...
			"--o", os.path.join(talon_analysis, os.path.basedir(file).replace(".genome.sam","_label")),  # Prefix for output files
			"--f", csv_file,  # Dataset config file: dataset name, sample description, platform, sam file (comma-delimited)
			"2>>", os.path.join(pipeline_reports, "talon_talon_priming_check-report.txt")])  # Directory where all reports reside
			subprocess.run(talon_priming_check, shell=True)

		### Second step: annotating and quantification of the reads
		print("{0}  2/6 TALON - Annotating and quantification of the reads: in progress ..".format(datetime.now().strftime("%d.%m.%Y %H:%M")))
		...
		"--f", csv_file,  # Dataset config file: dataset name, sample description, platform, sam file (comma-delimited)
		"2>>", os.path.join(pipeline_reports, "talon_annotationNquantification-report.txt")])  # Directory where all reports reside
		subprocess.run(talon_annotation, shell=True)
		
		### Third step: summarising how many of each transcript were found (prior to any filtering)
		print("{0}  3/6 TALON - Summarising how many of each transcript were found (prior to any filtering): in progress ..".format(datetime.now().strftime("%d.%m.%Y %H:%M")))
		talon_summary = " ".join([
		...
		"2>>", os.path.join(pipeline_reports, "talon_summary-report.txt")])  # Directory where all reports reside
		subprocess.run(talon_summary, shell=True)
		
		### Fourth step: creating an abundance matrix without filtering (for use computing gene expression)
		print("{0}  4/6 TALON - Creating an abundance matrix without filtering (for use computing gene expression): in progress ..".format(datetime.now().strftime("%d.%m.%Y %H:%M")))
		...
		"2>>", os.path.join(pipeline_reports, "talon_abundance-report.txt")])  # Directory where all reports reside
		subprocess.run(talon_abundance, shell=True)
		
		### Fifth step: Applying basic filtering steps and outputting several stats
		R_analysis = os.path.join(talon_analysis, "R_analysis")
		if not os.path.exists(R_analysis): os.makedirs(R_analysis)
		print("{0}  5/6 TALON - : in progress ..".format(datetime.now().strftime("%d.%m.%Y %H:%M")))
		...
		"2>>", os.path.join(pipeline_reports, "talon_summarisation.txt")])  # Directory where all reports reside
		...
		print("{0}  6/6 TALON - : in progress ..".format(datetime.now().strftime("%d.%m.%Y %H:%M")))
		talon_report = " ".join([
		...
		"--outdir", talon_analysis,  # Output dir
		"2>>", os.path.join(pipeline_reports, "talon_generate_report-report.txt")])  # Directory where all reports reside
	...
	def novel_transcripts_detection_pinfish(self, threads):
		...
		for file in polished_fasta:
			...
			# "2>>", os.path.join(pipeline_reports, "pinfish4_minimap2_genome-report.txt")
		...
		for file in clustered_polised_gffs:
			...
			# "2>>", os.path.join(pipeline_reports, "pinfish7_collapse_partials_polished-report.txt")
			])  # Directory where all reports reside
		...
		for file in collapsed_polised_gffs:
			...
			file,  # Clustered transcripts in gff format
			# "2>>", os.path.join(pipeline_reports, "pinfish7_gffread-report.txt")
			])  # Directory where all reports reside
		...
		# "2>>", os.path.join(pipeline_reports, "pinfish8_gffcompare-report.txt")
		])  # Directory where all reports reside
	...
	def novel_transcripts_detection_flair(self, threads, chosen_samples):
		...
		# 		batch = fastq_file.split("/")[-4]
		# 		flair_out.write('{0}\t{1}\t{2}\t{3}\n'.format(sample_id, sample_id.split("_")[0], batch,fastq_file))
		...
		# "--output", flair_analysis,  # output file
	...
	def novel_transcripts_detection_sqanti2(self, threads):
		...
		collapsed_unique_isoforms = [sum_file for sum_file in glob.glob(os.path.join(expression_analysis_dir, "pinfish_analysis/*.pinfish8.collapsed_unique_isoforms.fasta"))]
		...
		for file in collapsed_unique_isoforms:
			...
			# "2>>", os.path.join(pipeline_reports, "sqanti_qc2-report.txt")
			])  # Directory where all reports reside
...
def summary(num_of_files):
	...
	figures_pre_dir = os.path.join(initial_qc_reports, "figures")
	...
	for path, subdir, folder in os.walk(initial_qc_reports):
		for name in folder:
